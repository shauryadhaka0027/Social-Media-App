...
const SocialFeed = () => {
  ...
  console.log("activeCommentPostId", posts)
  ...
  const handleDelete = (data) => {
    deleteUserPost.mutate({postId:data},{onSuccess:(data)=>{
      getUserPostData();
      notification.success({
        type: "success",
  ...
  return (
    <div className="w-[100%] md:w-[45%] mx-auto bg-gray-50 p-4">
      ...
      {(userPost?.isPending || deleteUserPost?.isPending ) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          {/* Spinner */}
          <div
            className="inline-block h-12 w-12 animate-spin rounded-full border-4 border-blue-500 border-t-transparent"
            role="status"
          >
                  ...
                  <div key={comment._id} className="flex items-start gap-4 mt-4">
                    ...
                    <ul>
                      {notifications.map((notification, index) => (
                        <li key={index}>
                          {notification.type === "like" && `You have a new like on your post!`}
                          {/* {notification.type === "follow" && `You have a new follower!`} */}
                        </li>
                      ))}
                    </ul>
